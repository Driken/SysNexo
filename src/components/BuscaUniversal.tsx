...
export const BuscaUniversal: React.FC = () => {
  ...
  const buscarPacientes = async (q: string) => {
    ...
    if (filtros.includes('Nome')) orFilters.push(`nome.ilike.%${q}%`);
    ...
    <div className="glass-card" style={{ marginBottom: '2rem', position: 'relative', zIndex: isFilterOpen ? 10 : 1 }}>
            ...
            {isFilterOpen && (
              <div 
                style={{
                  ...
                  boxShadow: 'var(--shadow-xl)', padding: '1rem', zIndex: 1000, animation: 'fadeIn 0.2s'
                }}
              >
      ...
      <div className="input-group" style={{ position: 'relative' }}>
          <input 
            ...
            style={{ paddingLeft: '3rem', fontSize: '1.05rem', background: 'hsla(var(--bg-main), 0.5)' }}
            placeholder={viewMode === 'recepcao' ? "Verificar histórico (Nome, CPF ou SUS)..." : "Buscar paciente encaminhado..."} 
            value={query}
            onChange={e => setQuery(e.target.value)}
          />
          <Search size={22} color="hsl(var(--text-muted))" style={{ position: 'absolute', left: '1rem', top: '1rem' }} />
      </div>
