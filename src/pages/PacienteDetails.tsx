...
import { ArrowLeft, User, Plus, FileText, AlertCircle, Clock, ChevronUp, Cloud, Send, CheckCircle2, Activity } from 'lucide-react';
import { useAutoSave } from '../hooks/useAutoSave';
import { toast } from 'sonner';
import type { Profile, Agendamento } from '../lib/supabase';
...
export const PacienteDetails: React.FC = () => {
  ...
  const [novaNota, setNovaNota, clearNota, isNovaNotaSaved] = useAutoSave(`draft_nota_${id}`, '', 1000);
  const [novoPlano, setNovoPlano, clearPlano, isNovoPlanoSaved] = useAutoSave(`draft_plano_${id}`, '', 1000);
  ...
  const [agendamentoHoje, setAgendamentoHoje] = useState<Agendamento | null>(null);
  ...
  const carregarPsicologos = async () => {
    const { data } = await supabase.from('profiles')
      ...
      .eq('is_active', true);
    if (data) setPsicologos(data);
  };

  const checarAgendamentoHoje = async () => {
    ...
    setAgendamentoHoje(data as unknown as Agendamento);
  };
  ...
  const iniciarAtendimento = async () => {
    ...
    if (!error) {
      ...
      setIsFormOpen(true); // Abre o formulário automaticamente
      checarAgendamentoHoje();
    } else {
  ...
  const salvarSessao = async (e: React.FormEvent) => {
    ...
    if (!error && data) {
      ...
      console.error(error);
      toast.error("Erro ao salvar o prontuário. Reveja suas permissões.");
    }
    setSalvando(false);
  };
  ...
  return (
    <div>
      <div className="dashboard-header border-bottom">
        ...
        {/* Widget de Encaminhamento (Recepção/Admin) */}
        {(viewMode === 'recepcao' || viewMode === 'admin') && (
          <div style={{ marginTop: '1.5rem', padding: '1rem', background: 'hsla(var(--bg-main), 0.5)', borderRadius: 'var(--radius-sm)', border: '1px solid hsl(var(--border-light))' }}>
            {agendamentoHoje ? (
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', color: 'hsl(var(--primary))' }}>
                <CheckCircle2 size={18} />
                <span style={{ fontSize: '0.9rem', fontWeight: 600 }}>
                  Encaminhado hoje para: <strong>Dr(a). {agendamentoHoje.psicologo?.full_name}</strong> -
                  <span style={{ marginLeft: '0.5rem', textTransform: 'uppercase', fontSize: '0.75rem' }}>{agendamentoHoje.status}</span>
                </span>
              </div>
            ) : (
              <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem', flexWrap: 'wrap' }}>
                ...
                <div style={{ display: 'flex', gap: '0.75rem', flex: 1, minWidth: '300px' }}>
                  ...
                  >
                    <Send size={16} /> {encaminhando ? 'Encaminhando...' : 'Encaminhar'}
                  </button>
                </div>
              </div>
            )}
        ...
        {/* Widget de Início (Psicólogo) */}
        {(viewMode === 'psicologo') && agendamentoHoje && agendamentoHoje.status === 'Aguardando' && (
          <div style={{ marginTop: '1.5rem', padding: '1.5rem', background: 'hsla(var(--primary), 0.1)', borderRadius: 'var(--radius-sm)', border: '2px solid hsl(var(--primary))', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <h3 style={{ margin: 0, color: 'hsl(var(--primary))' }}>Paciente aguardando na recepção</h3>
              <p className="text-muted text-sm" style={{ margin: '0.25rem 0 0 0' }}>Você pode iniciar este atendimento agora para atualizar a fila.</p>
            </div>
            <button
              className="btn btn-primary"
            ...
            <div className="glass-card" style={{ marginTop: '1rem', padding: 0, overflow: 'hidden' }}>
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                  ...
                  {(isNovaNotaSaved || isNovoPlanoSaved) && (
                    <span style={{ fontSize: '0.75rem', color: '#10b981', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '0.35rem', animation: 'fadeIn 0.3s ease-out' }}>
                      <Cloud size={16} /> Rascunho salvo
                    </span>
                  )}
                </div>
